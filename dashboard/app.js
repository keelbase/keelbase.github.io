...
const dashboardTitleEl = document.querySelector("header h1");

const dashboardSection = new URLSearchParams(window.location.search).get("section") || "";
applySectionMode(dashboardSection);
...
function applySectionMode(sectionKey) {
  const section = String(sectionKey || "").trim().toLowerCase();
  if (!section) return;

  const map = {
    snapshot: ["cardSnapshot"],
    "latest-anchor": ["cardAnchor"],
    state: ["signalBox"],
    "recent-proposals": ["sectionProposals"],
    "launch-new-vessel": ["sectionLaunch"],
    "created-vessels": ["sectionVessels"],
    "talk-vessel-agent": ["sectionTalk"]
  };
  const allowed = new Set(map[section] || []);
  if (allowed.size === 0) return;

  if (dashboardTitleEl) {
    dashboardTitleEl.textContent = `Keelbase - ${section.replaceAll("-", " ")}`;
  }
  ...
  for (const id of candidates) {
    ...
    if (!node) continue;
    node.style.display = allowed.has(id) ? "" : "none";
  }
}
